...
public class LevelSalaryChange {
	...
	public static int getLevel() {
		...
		System.out.println("套改年限为" + salaryChangeTime + "年");
		...
		if (attendWorkDate.before(baseWorkDate)) {
			...
			if (techLength == 0) {
				...
				for (int i = 0; i < techLength; i++) {
					for (int j = i + 1; j < techLength; j++) {
						if (techList.get(i).getGanhuoLevel() < techList.get(j)
								.getGanhuoLevel()) {
			...
			if (manageMaxLevel == 0) {
				...
				if (manageSecMaxLevel == 0) {
					...
					int officeTime = getManageOfficeTime(manageMaxLevel,
							manageMinYear1, techList);
					System.out.println(officeTime);
					int curLevel = manageSalaryManage.getPayLevel(11-manageMaxLevel, officeTime, salaryChangeTime);
					if (curLevel < manageSalaryManage.findManPosSalByLevel(11-manageMaxLevel).getStartPayLevel())
						curLevel = manageSalaryManage.findManPosSalByLevel(11-manageMaxLevel).getStartPayLevel();
					if (curLevel > level)
						level = curLevel;
				} else {
					...
					int curLevel1 = manageSalaryManage.getPayLevel(11-manageMaxLevel, officeTime1, salaryChangeTime);
					if (curLevel1 < manageSalaryManage.findManPosSalByLevel(11-manageMaxLevel).getStartPayLevel())
						curLevel1 = manageSalaryManage.findManPosSalByLevel(11-manageMaxLevel).getStartPayLevel();
					if (curLevel1 > level)
						level = curLevel1;

					System.out.println("管理系列2：");
					int officeTime2 = getManageOfficeTime(manageSecMaxLevel,
							manageMinYear2, techList);
					System.out.println(officeTime2);
					int curLevel2 = manageSalaryManage.getPayLevel(11-manageSecMaxLevel, officeTime2, salaryChangeTime);
					if (curLevel2 < manageSalaryManage.findManPosSalByLevel(11-manageSecMaxLevel).getStartPayLevel())
						curLevel2 = manageSalaryManage.findManPosSalByLevel(11-manageSecMaxLevel).getStartPayLevel();
					if (curLevel2 > level)
						level = curLevel2;
				}
			}
			...
			if (techMaxLevel == 0) {
				...
				if (techSecMaxLevel == 0) {
					...
					int officeTime = getTechOfficeTime(techMaxLevel, techMinYear1,
							manageList);
					System.out.println(officeTime);
					int curLevel = profSalaryManage.getPayLevel(techMaxLevel, officeTime, salaryChangeTime);
					if (curLevel < profSalaryManage.findProfPosSalByLevel(14-techMaxLevel).getStartPayLevel())
						curLevel = profSalaryManage.findProfPosSalByLevel(14-techMaxLevel).getStartPayLevel();
					if (curLevel > level)
						level = curLevel;
				} else {
					...
					System.out.println(officeTime1);
					int curLevel1 = profSalaryManage.getPayLevel(techMaxLevel, officeTime1, salaryChangeTime);
					if (curLevel1 < profSalaryManage.findProfPosSalByLevel(14-techMaxLevel).getStartPayLevel())
						curLevel1 = profSalaryManage.findProfPosSalByLevel(14-techMaxLevel).getStartPayLevel();
					if (curLevel1 > level)
						level = curLevel1;
					...
					System.out.println(officeTime2);
					int curLevel2 = profSalaryManage.getPayLevel(techSecMaxLevel, officeTime2, salaryChangeTime);
					if (curLevel2 < profSalaryManage.findProfPosSalByLevel(14-techSecMaxLevel).getStartPayLevel())
						curLevel2 = profSalaryManage.findProfPosSalByLevel(14-techSecMaxLevel).getStartPayLevel();
					if (curLevel2 > level)
						level = curLevel2;
				}
			}
			...
			for (int i = 0; i < eduLength; i++) {
				if (eduList.get(i).getTime() >= 2006) {
					if (eduList.get(i).getGanhuo() == "专科") {
						if ((new Date().getYear() - eduList.get(i).getTime() + 5) > level)
							level = new Date().getYear() - eduList.get(i).getTime() + 5;
					} else if (eduList.get(i).getGanhuo() == "本科") {
						if ((new Date().getYear() - eduList.get(i).getTime() + 7) > level)
							level = new Date().getYear() - eduList.get(i).getTime() + 7;
					} else if (eduList.get(i).getGanhuo() == "双学士") {
						if ((new Date().getYear() - eduList.get(i).getTime() + 9) > level)
							level = new Date().getYear() - eduList.get(i).getTime() + 9;
					} else if (eduList.get(i).getGanhuo() == "硕士") {
						if ((new Date().getYear() - eduList.get(i).getTime() + 11) > level)
							level = new Date().getYear() - eduList.get(i).getTime() + 11;
					} else if (eduList.get(i).getGanhuo() == "博士") {
						if ((new Date().getYear() - eduList.get(i).getTime() + 14) > level)
							level = new Date().getYear() - eduList.get(i).getTime() + 14;
					}
				} else {
					if (eduList.get(i).getGanhuo() == "专科") {
