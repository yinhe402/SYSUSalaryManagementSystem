	...
	public static void main(String[] args) throws IOException {
		...
		while (yesorno == 1) {
			...
			if (breakWorkStartTime.before(breakWorkEndTime)) {
				breakTime = breakTime + dateDif(breakWorkStartTime, breakWorkEndTime);
			}
			else {
		...
		int realWorkTime = 2006 - (1900 + attendWorkDate.getYear()) + 1 - breakTime;//实际工作年限
		...
		if (attendWorkDate.before(baseWorkDate)) {
			...
			//管理系第一大，第二大
			ExperienceInfo techMax = new ExperienceInfo("", 0);
			ExperienceInfo techSecMax = new ExperienceInfo("", 0);
			
			//最高学历
			ExperienceInfo eduMax = new ExperienceInfo("", 0);
